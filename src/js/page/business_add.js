var vue_instance = new Vue({
    ...
    methods: {
        submit_form:function () {                        
            ...
            jquery_ajax_obj({"url":ACTION_URL.business_modify,"request_type":"post","post_data":this.form_data,"is_json_param":true,
                "callback_func":(json_result)=>{                
                    console.log(json_result);
                    alert("操作成功");
                    if(this.form_data.id > 0){
                         location.href="/production/business/index.html?current_page="+this.current_page;
                    }else{
                        location.href="/production/business/index.html";
                    }
                },
            });                               
        },        
        load_edit_data(){ //拉取修改页的数据   
            jquery_ajax_obj({"url":ACTION_URL.business_get_detail,"request_type":"post","post_data":this.form_data.id,"is_json_param":false,
                "callback_func":(json_result)=>{
